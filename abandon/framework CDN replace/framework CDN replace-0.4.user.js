...
(function() {
    ...
    for (i = 0; i < Aims.length; i += 1) {
        ...
        try {
            rep = Aims[i].src;
            Aim.setAttribute('src', rep.replace(reg_cdnjs, 'https://cdn-lib.oss-cn-beijing.aliyuncs.com/cdnjs'));
        }catch(TypeError) {
            rep = Aims[i].href;
            Aim.setAttribute('href', rep.replace(reg_cdnjs, 'https://cdn-lib.oss-cn-beijing.aliyuncs.com/cdnjs'));
        }
    }
})();